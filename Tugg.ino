...
#include <FastLED.h>
// Running on an Arduino Nano, use with TUGG PCB 2 v30 board (or higher)
// can add movement to lights and additional filters later

#define RELAY_SWITCH_PIN 8         // motor power cutoff relay control
#define LED_CTRL_PIN     7         // LED control pin for lightstrip
#define ULTRASONIC_ECHO_PIN 10     // ultrasonic rx   
#define ULTRASONIC_TRIGGER_PIN 11  // ultrasonic tx 
#define RECEIVER_CH1_INPUT 6       // motor left right
#define RECEIVER_CH2_INPUT 5       // motor forward backward
#define RECEIVER_CH6_INPUT 3       // set distance input
...
void config_pins()
{
  ...
  pinMode(LED_CTRL_PIN, OUTPUT);              // DIN pin of LED Strip
...
void setup(){
  ...
  FastLED.addLeds<LED_TYPE, LED_CTRL_PIN, COLOR_ORDER>(leds, NUM_LEDS);        // LED strip number of LEDs
  FastLED.setBrightness(BRIGHTNESS);      // LED strip intial brightness
...
void loop(){
int stop_distance=90; // initial stop_distance 
int f_b_value =   100   ;  // initial forward/backward
int l_r_value =   100   ;  // initial laft/right
  ...
  for(int i=0; i<=NUM_LEDS; i++){  //Turn all LEDs Blue for now; can make more complex later  
      leds[i].setRGB(0,0,255);  
}
  FastLED.show();
  
  int us_dist_last = us_dist;
  int us_dist = map(ultrasonic.read(CM), 0, 260, 50, 200); 
  if (us_dist > 260) { //Do not allow out-of-range values
    us_dist=260;
  }
  /*         IMPROVE THIS FILTER
  ...
  */
  char printvar[100];
  sprintf(printvar,"StopDistance: %03d, US_Dist: %03d, F/B: %03d, L/R: %03d \n", stop_distance, us_dist, f_b_value, l_r_value );
